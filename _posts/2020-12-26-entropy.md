...
Let us consider some trajectory _ϕ(__r__,t)_ from _t=t<sup>i</sup>_ to _t=t<sup>f</sup>_. The rate of entropy production _S_ is defined to be the logarithm of the ratio forward probability to the backwards probability:

<img src="http://latex.codecogs.com/svg.latex?S=\lim_{\tau\rightarrow\infty}\frac{1}{\tau}\left<\ln\left(\frac{P_F[\phi]}{P_B[\phi]}\right)\right>" border="0"/>
