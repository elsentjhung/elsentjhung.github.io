...
However, when we perform the simulations, we do it in `Ito’ way. i.e. we start from $\phi(\mathbf{r},0)$, then we update the time to get $\phi(\mathbf{r},0), \phi(\mathbf{r},dt), \phi(\mathbf{r},2dt), \dots$. 
